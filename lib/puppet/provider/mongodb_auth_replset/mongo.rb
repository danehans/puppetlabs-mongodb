...
Puppet::Type.type(:mongodb_auth_replset).provide(:mongo) do
  ...
  def members_present
    @resource[:members].select do |host|
      begin
        self.mongo(@resource[:admin_db], '--host', host, '-u', @resource[:user], '-p', @resource[:password], '--quiet', '--eval', 'db.version()')
        true
      rescue Puppet::ExecutionFailure
        false
      end
  ...
  def mongo_command(command, host, retries=4)
    ...
    wait = 2
    begin
      output = self.mongo(@resource[:admin_db], '--quiet', '--host', host, '-u', @resource[:user], '-p', @resource[:password], '--eval', "printjson(#{command})")
  ...
  def mongo_nohost_command(command, retries=4)
    # Allow waiting for mongod to become ready
    # Wait for 2 seconds initially and double the delay at each retry
    wait = 2
    begin
      output = self.mongo(@resource[:admin_db], '-u', @resource[:user], '-p', @resource[:password], '--quiet', '--eval', "printjson(#{command})")
      sleep 60
    rescue Puppet::ExecutionFailure => e
      if e =~ /Error: couldn't connect to server/ and wait <= 2**max_wait
        info("Waiting #{wait} seconds for mongod to become available")
  ...
  def rs_initiate(conf, host)
    return self.mongo_nohost_command("rs.initiate(#{conf})", @resource[:members][0])
  end
